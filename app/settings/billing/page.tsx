  ...
  async function loadSubscription() {
    try {
      ...
      const res = await fetch(
        `${API_BASE}/billing/subscriptions/customer/${customerId}`,
      );
      if (res.ok) {
        const data = await res.json();
    ...
    <div className="flex flex-col gap-6 p-6 max-w-6xl">
      ...
      <header>
        <h1 className="text-2xl font-semibold text-white flex items-center gap-2">
          <CreditCard className="w-6 h-6 text-blue-400" weight="duotone" />
        ...
        <div className="rounded-lg border border-neutral-800 bg-neutral-900/50 p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <Button onClick={manageBilling} variant="outline">
              <ArrowSquareOut className="w-4 h-4 mr-2" weight="bold" />
              Manage Billing
            </Button>
          </div>
